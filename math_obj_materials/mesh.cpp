...
{	
	...
	Vector4DF palette[8];
	palette[0] = Vector4DF(0.5, 0.5, 0.5, 1);
	palette[1] = Vector4DF(1,0,0,1);
	palette[2] = Vector4DF(0,1,0,1);
	palette[3] = Vector4DF(1,1,0,1);
	palette[4] = Vector4DF(0,0,1,1);
	palette[5] = Vector4DF(1,0,1,1);
	palette[6] = Vector4DF(0,1,1,1);
	palette[7] = Vector4DF(1,1,1,1);
		...
		case 'f':
			...
			for (int j=0; j < 3; j++ ) {				
				...
				if ( vntmap.find ( triple_t( v[j], n[j], t[j] ) ) == vntmap.end() ) {
					...
					Vector4DF c = palette[ curr_mtl % 8 ];
