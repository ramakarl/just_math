...
bool Sample::init()
{
	...
	m_cam->setOrbit(Vector3DF(-40, 30, 0), Vector3DF(32, 32, 32), 400, 70);
	m_adjust = 0;
...
void Sample::display()
{
	...
	drawText(10, 20, "Voxelizer (c) Rama 2021, CC-BY", 1, 1, 1, 1);
	sprintf(buf, "Algorithm (press 1,2,3,4): %s", msg);			drawText(10, 60, buf, 1, 1, 1, 1);
	sprintf(buf, "Move the yellow vertices with left-click drag.");	drawText(10, 90, buf, 1, 1, 1, 1);
	sprintf(buf, "Rotate the camera with right drag.");			drawText(10, 120, buf, 1, 1, 1, 1);
	sprintf(buf, "Voxels total:    %d", m_vres.x * m_vres.y * m_vres.z); drawText(10, 150, buf, 1, 1, 1, 1);
	sprintf(buf, "Voxels visited:  %d", m_voxel_visit);	drawText(10, 170, buf, 1, 1, 1, 1);
	sprintf(buf, "Voxels set:      %d", m_voxel_set);	drawText(10, 190, buf, 1, 1, 1, 1);
	sprintf(buf, "Voxels active:   %d", m_voxel_count);	drawText(10, 210, buf, 1, 1, 1, 1);
	end();
	...
	setdepth3D(false);
	setLight(S3D, 20, 100, 20);
	drawCircle3D (tri[0], tri[0] + Vector3DF(0, 1, 0), 2.0, Vector4DF(1, 1, (m_adjust == 0) ? 1 : 0, 1) );	// corners of triangle (interactive)
	drawCircle3D (tri[1], tri[1] + Vector3DF(0, 1, 0), 2.0, Vector4DF(1, 1, (m_adjust == 1) ? 1 : 0, 1) );
	drawCircle3D (tri[2], tri[2] + Vector3DF(0, 1, 0), 2.0, Vector4DF(1, 1, (m_adjust == 2) ? 1 : 0, 1) );
	end();
...
void Sample::motion(AppEnum btn, int x, int y, int dx, int dy)
{
	...
	case AppEnum::BUTTON_LEFT: {
		...
		if ( m_adjust >= 0  ) {
			tri[m_adjust] = moveHit3D(m_cam, x, y, tri[m_adjust], Vector3DF(0, 1, 0));	// xz plane			
			Voxelize();
		}
	...
	case AppEnum::BUTTON_RIGHT: {
		...
		if (m_adjust >= 0) {
			tri[m_adjust] = moveHit3D(m_cam, x, y, tri[m_adjust], Vector3DF(0, 0, 1));		// xy plane			
			Voxelize();
		}
