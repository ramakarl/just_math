...
void Image::SetPixelRGB16 (int x, int y, XBYTE r, XBYTE g, XBYTE b, XBYTE a)
{
	if ( x>=0 && y>=0 && x < getInfo()->mXres && y < getInfo()->mYres ) {
		XBYTE2* pix = (XBYTE2*) GetData() + ( (y * getInfo()->mXres + x) * getInfo()->GetBytesPerPix() );
		*pix++ = r;
		*pix++ = g;
		*pix++ = b;
...
void Image::GetPixelRGB16 (int x, int y, XBYTE& r, XBYTE& g, XBYTE& b, XBYTE& a )
{
	if ( x>=0 && y>=0 && x < getInfo()->mXres && y < getInfo()->mYres ) {
		XBYTE2* pix = (XBYTE2*) GetData() + ( (y * getInfo()->mXres + x) * getInfo()->GetBytesPerPix() );
        r = *pix++;
...
void Image::FillRGB16 (XBYTE r, XBYTE g, XBYTE b, XBYTE a)
{
...
void Image::RemapRGB16 ( unsigned int vmin, unsigned int vmax )
{
	XBYTE2* src = (XBYTE2*) GetData();
	XBYTE2* src_stop = src + (getInfo()->mXres*getInfo()->mYres);	
...
// Reformat - Reformats given pixel format RGB16 to
// another pixel format
void Image::ReformatRGB16 ( ImageOp::Format eFormat )
{
	Image* new_img = new Image (  getInfo()->mXres, getInfo()->mYres, eFormat );
	
	XBYTE2* src = (XBYTE2*) GetData();
	XBYTE2* src_stop = src + getInfo()->GetSize();	
...
void Image::PasteRGB16 ( int x1, int y1, int x2, int y2, int offx, int offy, XBYTE* dest, ImageOp::Format dest_format, int destx, int desty )
{
...
void Image::ScaleRGB16 ( XBYTE* dest, ImageOp::Format dest_format, int destx, int desty )
{
...
void Image::AlphaRGB16 ( int x1, int y1, int x2, int y2, XBYTE* src, ImageOp::Format src_format, int src_x, int src_y )
{
	// No alpha channel !
