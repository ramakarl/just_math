...
void Image::SetPixelBGR8 (int x, int y, XBYTE r, XBYTE g, XBYTE b, XBYTE a)
{
...
void Image::GetPixelBGR8 (int x, int y, XBYTE& r, XBYTE& g, XBYTE& b, XBYTE& a )
{
	if ( x>=0 && y>=0 && x < getInfo()->mXres && y < getInfo()->mYres ) {
		XBYTE* pix = (XBYTE*) GetData() + ( (y * getInfo()->mXres + x) * getInfo()->mBitsPerPix >> 3);
        b = *pix++;
...
void Image::FillBGR8 (XBYTE r, XBYTE g, XBYTE b, XBYTE a)
{
...
void Image::RemapBGR8 ( unsigned int vmin, unsigned int vmax )
{
...
void Image::ReformatBGR8 ( ImageOp::Format eFormat )
{
	...
	if ( eFormat==ImageOp::RGBA8 ) {		// Target format RGBA8
		for (; src < src_stop; ) {
			*dest++ = *(src++ + 2);
			*dest++ = *src++;
			*dest++ = *(src++ - 2);
	...
	} else if ( eFormat==ImageOp::RGB8 ) {	// Target format RGB8
		for (; src < src_stop; ) {
			*dest++ = *(src++ + 2);
			*dest++ = *src++;
			*dest++ = *(src++ - 2);
...
void Image::PasteBGR8 ( int x1, int y1, int x2, int y2, int offx, int offy, XBYTE* dest, ImageOp::Format dest_format, int destx, int desty )
{
...
void Image::ScaleBGR8 ( XBYTE* dest, ImageOp::Format dest_format, int destx, int desty )
{
...
void Image::AlphaBGR8 ( int x1, int y1, int x2, int y2, XBYTE* src, ImageOp::Format src_format, int src_x, int src_y )
{
	// No alpha channel !
