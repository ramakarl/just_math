...
void Image::SetPixelBW16 (int x, int y, XBYTE r, XBYTE g, XBYTE b, XBYTE a)
{
...
void Image::GetPixelBW16 (int x, int y, XBYTE& r, XBYTE& g, XBYTE& b, XBYTE& a )
{
	if ( x>=0 && y>=0 && x < getInfo()->mXres && y < getInfo()->mYres ) {
...
void Image::FillBW16 (XBYTE r, XBYTE g, XBYTE b, XBYTE a)
{
...
void Image::RemapBW16 ( unsigned int vmin, unsigned int vmax )
{
	...
	for (; src < src_stop; ) {
		*src = (XBYTE2) ( float(vmin) + float(*src - mMin)*vdelta / (mMax-mMin) );
		src++;
	}
}
...
void Image::ReformatBW16 ( ImageOp::Format eFormat )
{
	...
	case ImageOp::BW16:		
		break;
	case ImageOp::BW32: {
		XBYTE4* dest = (XBYTE4*) new_img->GetData ();		
		for (; src < src_stop; ) {
			*dest++ = *src++;			
		}		
		} break;
...
void Image::PasteBW16 ( int x1, int y1, int x2, int y2, int offx, int offy, XBYTE* dest, ImageOp::Format dest_format, int destx, int desty )
{
...
void Image::ScaleBW16 ( XBYTE* dest, ImageOp::Format dest_format, int destx, int desty )
{
...
void Image::AlphaBW16 ( int x1, int y1, int x2, int y2, XBYTE* src, ImageOp::Format src_format, int src_x, int src_y )
{
